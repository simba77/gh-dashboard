...
export async function fetchProjectItemsPage(
  ...
  if (!project?.items) {
    return { projectTitle: '', items: [], hasMore: false, nextCursor: null };
  }
...
// Walks all pages of a project's items. Returns the aggregated list so the
// caller can decide what to store (full sync replaces, tail sync upserts).
export async function fetchAllProjectItems(projectId: string): Promise<UpsertItem[]> {
  const all: UpsertItem[] = [];
